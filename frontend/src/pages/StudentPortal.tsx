...
const StudentPortal: React.FC = () => {
    ...
    const getStatusStyles = (status: string) => {
        switch (status) {
            case 'ACCEPTED': return { icon: <CheckSquare className="w-12 h-12" />, color: 'text-edu-coral', bg: 'bg-edu-coral/10' };
            case 'REJECTED': return { icon: <ShieldAlert className="w-12 h-12" />, color: 'text-rose-500', bg: 'bg-rose-500/10' };
            case 'PENDING': return { icon: <Timer className="w-12 h-12" />, color: 'text-edu-yellow', bg: 'bg-edu-yellow/10' };
            default: return { icon: <FolderOpen className="w-12 h-12" />, color: 'text-blue-500', bg: 'bg-blue-500/10' };
        }
    };

    if (loading) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-brand-cream dark:bg-slate-950">
            <div className="w-16 h-16 border-4 border-edu-coral border-t-transparent rounded-full animate-spin" />
    ...
    if (!user) return (
        <div className="min-h-screen flex items-center justify-center bg-brand-cream dark:bg-slate-950">
            <div className="text-center">
                <h2 className="text-2xl font-black font-outfit text-brand-deep dark:text-white mb-4 tracking-tight">Unauthorized Session</h2>
    ...
    return (
        <div className="min-h-screen bg-brand-cream dark:bg-slate-950 pt-32 pb-20 px-4 transition-colors duration-500">
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                ...
                >
                    <div className="flex flex-col md:flex-row items-center gap-10">
                        <div className="relative">
                            <div className="w-28 h-28 bg-brand-cream rounded-[2.5rem] flex items-center justify-center shadow-2xl rotate-3 overflow-hidden p-3 border border-slate-100">
                            ...
                            <div className="absolute -bottom-2 -right-2 bg-white dark:bg-slate-900 p-2 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700">
                                <Sparkles className="w-5 h-5 text-edu-teal" />
                            </div>
                        </div>
                        <div className="text-center md:text-left">
                            <h1 className="text-4xl md:text-5xl font-black text-brand-deep dark:text-white font-outfit tracking-tighter mb-2">
                                Assalam-u-Alaikum, <span className="text-transparent bg-clip-text bg-gradient-to-r from-edu-coral to-edu-teal dark:from-edu-teal dark:to-edu-teal">{user.name.split(' ')[0]}</span>
                            </h1>
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                                ...
                                <span className="px-4 py-1.5 bg-edu-coral/10 dark:bg-edu-teal/10 text-edu-coral dark:text-edu-teal rounded-full text-xs font-black uppercase tracking-widest border border-edu-coral/20 dark:border-edu-teal/20">Post-Hifz 2026</span>
                            </div>
                        </div>
                        <div className="md:ml-auto">
                            <button className="p-4 bg-slate-100 dark:bg-slate-900 text-slate-400 dark:text-slate-500 rounded-2xl relative hover:text-edu-coral dark:hover:text-edu-teal transition-colors">
                                <Bell size={24} />
                ...
                {!appData ? (
                    ...
                    >
                        ...
                        <h2 className="text-3xl font-black text-brand-deep dark:text-white mb-4 tracking-tight font-outfit">Your Journey Awaits</h2>
                    ...
                    <div className="grid lg:grid-cols-3 gap-10">
                        ...
                        >
                            <div className="glass-card p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 relative group overflow-hidden">
                                <div className="absolute top-0 left-0 w-2 h-full bg-edu-coral dark:bg-edu-teal" />
                                <h3 className="text-2xl font-black text-brand-deep dark:text-white mb-8 font-outfit tracking-tight">Application Dossier</h3>
                                <div className="grid gap-6">
                                    {[
                                        ...
                                        <div key={i} className="flex justify-between items-center py-4 border-b border-slate-50 dark:border-slate-800 last:border-0">
                                            ...
                                            <span className={`font-bold text-lg ${row.highlight ? 'text-edu-coral dark:text-edu-teal' : row.success ? 'text-edu-teal' : 'text-slate-800 dark:text-slate-200'}`}>
                                                {row.value}
                                            </span>
                                        </div>
                                    ))}
                        ...
                        >
                            <div className="glass-card p-10 rounded-[2.5rem] text-center border border-slate-100 dark:border-slate-800 relative group">
                                ...
                                <div className="mt-6 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                                    <p className="text-sm font-medium text-slate-600 dark:text-slate-300 leading-relaxed">
                                        {appData.status === 'PENDING' ? 'Your sacred application is currently undergoing scholarly review at Darussalam Village.' :
                            ...
                            {appData.status === 'ACCEPTED' && (
                                <motion.button
                                    ...
                                    className="w-full bg-edu-teal dark:bg-edu-teal text-white dark:text-slate-950 py-6 rounded-[2rem] font-black text-lg shadow-2xl shadow-edu-teal/30 flex items-center justify-center gap-3 uppercase tracking-widest"
                                >
