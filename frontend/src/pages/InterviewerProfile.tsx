...
import { User, Mail, Phone, Save, ArrowLeft, Loader2, Camera, Trash2, Briefcase, Fingerprint } from 'lucide-react';
...
const InterviewerProfile: React.FC = () => {
            ...
            try {
                ...
                if (interviewer) {
                    setFormData({
                        ...
                        whatsapp: interviewer.user?.whatsapp || user?.whatsapp || '',
                        speciality: interviewer.speciality || '',
                        profileImageUrl: interviewer.user?.profileImageUrl || '',
                        profileImagePublicId: interviewer.user?.profileImagePublicId || '',
                    });
                }
            } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            // Update Interviewer specific data
            await api.patch('/interviews/me', { 
                name: formData.name, 
            ...
            // Update User generic data (redundant but ensures sync)
    ...
    if (authLoading || loading) {
        return (
             <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-slate-950">
                ...
                <p className="text-slate-500 font-black tracking-widest uppercase text-[10px]">Processing Interviewer Identity...</p>
            </div>
        );
    }

    return (
        <div className="min-h-screen bg-tharqiya-cream dark:bg-slate-950 pt-10 pb-20 px-4 transition-colors duration-500">
            <div className="max-w-4xl mx-auto space-y-10">
                ...
                >
                    <div className="space-y-4">
                        <Link to="/interviewer" className="inline-flex items-center gap-2 text-slate-500 hover:text-tharqiya-orange transition-colors font-black uppercase text-[10px] tracking-widest">
                        ...
                        <h1 className="text-4xl sm:text-5xl font-black text-tharqiya-deep dark:text-white font-outfit tracking-tighter uppercase leading-none">
                            Scholar <span className="text-tharqiya-orange">Evaluation</span>
                        </h1>
                        <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">Evaluation Specialist Profile</p>
                    </div>
                </motion.div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    >
                        <div className="absolute top-0 right-0 w-40 h-40 bg-tharqiya-orange/5 blur-3xl rounded-full" />
                            ...
                            <div className="relative group/photo">
                                ...
                                >
                                    {formData.profileImageUrl ? (
                                        ...
                                        <div className="flex flex-col items-center text-slate-300 dark:text-slate-700">
                                            <User size={48} />
                                            <span className="text-[8px] font-black uppercase tracking-widest mt-2">Interviewer</span>
                                        </div>
                                    )}
                                ...
                                {uploading && (
                                    <div className="absolute inset-0 bg-white/60 dark:bg-slate-900/60 backdrop-blur-md rounded-[2.5rem] flex items-center justify-center">
                                        <Loader2 className="w-8 h-8 text-tharqiya-orange animate-spin" />
                                    </div>
                                )}
                            </div>
                            <div className="flex-grow space-y-4">
                                <div className="flex items-center justify-center sm:justify-start gap-3">
                                    <h3 className="text-2xl sm:text-3xl font-black text-tharqiya-deep dark:text-white font-outfit uppercase tracking-tighter">{formData.name}</h3>
                                    <div className="px-3 py-1 bg-tharqiya-orange/10 text-tharqiya-orange text-[8px] font-black uppercase tracking-[0.2em] rounded-full border border-tharqiya-orange/20">
                                        Specialist
                                    </div>
                                </div>
                                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed max-w-sm">Your professional evaluator profile used for student interviews and academic assessments.</p>
                                <div className="flex flex-wrap justify-center sm:justify-start gap-4 pt-2">
                                    <button type="button" onClick={() => (document.querySelector('input[type="file"]') as HTMLInputElement)?.click()} className="px-6 py-3 bg-tharqiya-deep dark:bg-white text-white dark:text-tharqiya-deep rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-tharqiya-orange hover:text-white transition-all shadow-lg">Change Photo</button>
                                    {formData.profileImageUrl && (
                    ...
                    >
                        <div className="flex items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-8">
                            <div className="p-4 bg-tharqiya-orange/10 text-tharqiya-orange rounded-2xl">
                                <Fingerprint size={24} />
                            </div>
                            <div>
                                <h2 className="text-xl sm:text-2xl font-black text-tharqiya-deep dark:text-white font-outfit uppercase tracking-tighter">Evaluation Identity</h2>
                                <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Professional Identification</p>
                            </div>
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-8 sm:gap-12">
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl font-black text-slate-800 dark:text-white focus:ring-4 focus:ring-tharqiya-orange/10 outline-none transition-all"
                                    />
                                </div>
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Email (Read Only)</label>
                                <div className="relative">
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">WhatsApp Number</label>
                                <div className="relative">
                                    <Phone className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" size={18} />
                                    <input 
                                        type="tel" 
                            ...
                            <div className="space-y-3 col-span-full">
                                <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Specialization</label>
                                <div className="relative">
                                    <Briefcase className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" size={18} />
                                    <input 
                                        type="text" 
                                        name="speciality"
                        ...
                        <div className="pt-10 border-t border-slate-100 dark:border-slate-800 flex justify-end">
                            <motion.button 
                                ...
                                type="submit" 
                                disabled={saving}
                                className="flex items-center gap-3 px-12 py-5 bg-tharqiya-orange text-white rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl shadow-tharqiya-orange/25 hover:bg-tharqiya-deep transition-all disabled:opacity-50 disabled:cursor-not-allowed text-xs"
                            >
